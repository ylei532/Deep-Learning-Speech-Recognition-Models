...
class TransformerEncoder(layers.Layer):
    def __init__(self, embed_dim, num_heads, feed_forward_dim, dropout_rate=0.1):
        ...
        self.ffn = keras.Sequential(
            [
                layers.Conv1D(feed_forward_dim, 11, strides=1, padding="same", activation="relu"),
                layers.Conv1D(embed_dim, 11, strides=1, padding="same"),
            ]
        )
...
class TransformerDecoder(layers.Layer):
    ...
    def call(self, enc_out, target):
        ...
        out1 = target + self.self_dropout(target_att)
        out1_norm = self.layernorm2(out1)
        enc_out = self.enc_att(out1_norm, enc_out)
        out2 = out1 + enc_out
        out2_norm = self.layernorm3(out2)
        ffn_out = self.ffn(out2_norm)
        return out2 + ffn_out
...
class Transformer(keras.Model):
    def __init__(
        ...
        self.model_name = 'TransformerModel5'
