...
class TransformerDecoder(layers.Layer):
    def __init__(self, embed_dim, num_heads, feed_forward_dim, dropout_rate=0.1):
        ...
        self.left_SepConv1D = layers.SeparableConv1D(self.embed_dim, 11, strides=1, padding="same", activation="relu")
        self.right_SepConv1D = layers.SeparableConv1D(self.embed_dim, 11, strides=1, padding="same")
        self.last_SepConv1d = layers.Dense(self.embed_dim)
    ...
    def call(self, enc_out, target):
        ...
        target_att = self.self_att(target_norm, target_norm, attention_mask=causal_mask)
        enc_out = self.enc_att(target_norm, enc_out)
        out1 = self.enc_dropout(enc_out) + self.self_dropout(target_att) + target

        out1_norm = self.layernorm2(out1)
        ffn_out = self.left_SepConv1D(out1_norm) + self.right_SepConv1D(out1_norm)

        ffn_out = self.layernorm3(self.ffn_dropout(ffn_out))
        return out1 + self.last_SepConv1d(ffn_out)
...
class Transformer(keras.Model):
    def __init__(
        ...
        self.model_name = 'TransformerModel3'
